...
OUT=Path('anime_primary_retry4_assets'); OUT.mkdir(exist_ok=True)
ZIP=Path('anime_primary_retry4_assets_png_max130kb.zip'); PREVIEW=Path('anime_primary_retry4_preview.jpg')
ASSETS=[
...
def franchise_ok(series,titles):
    ser=norm(series)
    aliases={'demon slayer':['kimetsu no yaiba'],'attack on titan':['shingeki no kyojin'],'pokemon':['pokemon','pocket monsters'],'jojo s bizarre adventure':['jojo','jojo no kimyou na bouken'],'re zero':['re zero','re zero kara hajimeru'],'frieren beyond journey s end':['frieren','sousou no frieren'],'rurouni kenshin':['rurouni kenshin','samurai x'],'saint seiya':['saint seiya'],'ghost in the shell':['ghost in the shell'],'mob psycho 100':['mob psycho'],'jujutsu kaisen':['jujutsu kaisen'],'chainsaw man':['chainsaw man'],'code geass':['code geass'],'sword art online':['sword art online'],'trigun':['trigun'],'death note':['death note'],'one punch man':['one punch man']}
    for t in titles:
        nt=norm(t)
...
def lookup(search):
    ...
    for attempt in range(7):
        r=requests.post(API,json={'query':QUERY,'variables':{'search':search}},headers={'User-Agent':UA,'Accept':'application/json','Content-Type':'application/json'},timeout=50)
        last=f'{r.status_code} {r.text[:300]}'
        if r.status_code==200:
            p=r.json(); return (p.get('data') or {}).get('Character'), p.get('errors') or []
        # GraphQL 404 can simply mean no exact match; don't hammer it forever.
        if r.status_code==404:return None, r.json().get('errors') or []
        if r.status_code==429:
            wait=int(r.headers.get('Retry-After') or 65); print(f'Rate limit: sleeping {wait}s',flush=True);time.sleep(wait);continue
        if r.status_code in (500,502,503,504):time.sleep(10+attempt*5);continue
        r.raise_for_status()
    raise RuntimeError('lookup failed: '+last)
...
for i,(filename,entity,series,search) in enumerate(ASSETS,1):
    print(f'[{i}/{len(ASSETS)}] {entity} search={search}',flush=True);c=None;selected='';notes='';status='PENDING_NOT_FOUND';titles=[]
    try:
        c,errs=lookup(search)
        if not c:raise RuntimeError('AniList no result: '+str(errs))
        for m in ((c.get('media') or {}).get('nodes') or []):
            t=m.get('title') or {};titles += [x for x in (t.get('english'),t.get('romaji'),t.get('native')) if x]
        if not franchise_ok(series,titles):raise RuntimeError('Series verification failed: '+' | '.join(titles[:8]))
        ...
        notes=f'Slow AniList lookup; search={search}; matched={c.get("name",{}).get("full")}; palette={colors}; media='+' | '.join(titles[:6])
...
with (OUT/'manifest.csv').open('w',newline='',encoding='utf-8') as f:w=csv.DictWriter(f,fieldnames=manifest[0].keys());w.writeheader();w.writerows(manifest)
with (OUT/'SEARCH_REPORT.csv').open('w',newline='',encoding='utf-8') as f:w=csv.DictWriter(f,fieldnames=report[0].keys());w.writeheader();w.writerows(report)
with (OUT/'ASSET_ATTRIBUTION.csv').open('w',newline='',encoding='utf-8') as f:w=csv.DictWriter(f,fieldnames=attrib[0].keys() if attrib else ['filename']);w.writeheader();w.writerows(attrib)
