...
OUT=Path('anime_primary_retry3_assets'); OUT.mkdir(exist_ok=True)
ZIP=Path('anime_primary_retry3_assets_png_max130kb.zip'); PREVIEW=Path('anime_primary_retry3_preview.jpg')
ASSETS=[
...
def franchise_ok(series,titles):
    ser=norm(series)
    aliases={
        'demon slayer':['kimetsu no yaiba'],'attack on titan':['shingeki no kyojin'],'pokemon':['pokemon','pocket monsters'],
        'jojo s bizarre adventure':['jojo','jojo no kimyou na bouken'],'re zero':['re zero','re zero kara hajimeru'],'frieren beyond journey s end':['frieren','sousou no frieren'],
        'rurouni kenshin':['rurouni kenshin','samurai x'],'saint seiya':['saint seiya'],'ghost in the shell':['ghost in the shell'],'mob psycho 100':['mob psycho'],
        'jujutsu kaisen':['jujutsu kaisen'],'chainsaw man':['chainsaw man'],'code geass':['code geass'],'sword art online':['sword art online'],'trigun':['trigun'],'death note':['death note'],
...
def query_chunk(chunk):
    ...
    for local_i,item in enumerate(chunk):
        search=item[3]
        fields.append(f'''c{local_i}: Character(search:"{esc(search)}"){{id name{{full native alternative}} image{{large medium}} siteUrl media(perPage:10){{nodes{{title{{romaji english native}}}}}}}}''')
    query='query{'+ '\n'.join(fields) +'}'
    last=''
    for attempt in range(6):
        try:
            r=requests.post(API,json={'query':query},headers={'User-Agent':UA,'Accept':'application/json','Content-Type':'application/json'},timeout=70)
            last=f'HTTP {r.status_code}: {r.text[:240]}'
            if r.status_code==200:
                payload=r.json(); return payload.get('data') or {}, payload.get('errors') or []
            if r.status_code in (404,429,500,502,503,504):
                time.sleep(5+attempt*4); continue
            r.raise_for_status()
        except Exception as e:
            last=str(e); time.sleep(5+attempt*4)
    raise RuntimeError('AniList chunk query failed after retries: '+last)
...
results={}; errors={}
CHUNK=8
for start in range(0,len(ASSETS),CHUNK):
    chunk=ASSETS[start:start+CHUNK]
    print(f'Querying chunk {start//CHUNK+1}: {start+1}-{start+len(chunk)}',flush=True)
    data,errs=query_chunk(chunk)
    if errs: print('GraphQL errors:',errs,flush=True)
    for local_i,item in enumerate(chunk): results[start+local_i]=data.get(f'c{local_i}')
    time.sleep(7)

manifest=[]; report=[]; attrib=[]; ok=[]
for i,(filename,entity,series,search) in enumerate(ASSETS):
    c=results.get(i); selected=''; status='PENDING_NOT_FOUND'; notes=''; titles=[]
    try:
        ...
        if not franchise_ok(series,titles): raise RuntimeError('Series verification failed: '+' | '.join(titles[:6]))
        selected=(c.get('image') or {}).get('large') or (c.get('image') or {}).get('medium')
        if not selected: raise RuntimeError('no image URL')
        ...
        notes=f'Chunked AniList lookup; search={search}; matched={c.get("name",{}).get("full")}; palette={colors}; media='+' | '.join(titles[:5])
